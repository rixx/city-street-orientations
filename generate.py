...
def get_bearing(place):
    ...
    if weight_by_length:
        ...
        streets = [(d['bearing'], int(d['length'])) for u, v, k, d in graph.edges(keys=True, data=True)]
        city_bearings = []
        ...
        return pd.Series([data['bearing'] for u, v, k, data in graph.edges(keys=True, data=True)])


if __name__ == '__main__':
    gdf = ox.gdf_from_places(places.values())
    bearings = {place: get_bearing(place) for place in sorted(places.keys())}
    bearings = {key: value for key, value in bearings.items() if value is not None}
